# Scan status — 2026-09-06

This repository has no automated vulnerability gate. Before publishing, the operator
must scan both architectures, resolve every finding with an available upstream fix,
and retain the unfiltered results. Findings with no fix are tracked here and in
`compliance/vulnerability-findings.csv`.

| Image | Architecture | Last recorded result |
| --- | --- | --- |
...
libpng16-16t64, libuuid1, and zlib1g. The tracked register is
`compliance/vulnerability-findings.csv`. Some
...
- `python3 scripts/dependencies.py --update` refreshes the base digest; rebuilding and
  publishing remain manual.
- Retain each unfiltered scan report with the corresponding release evidence.
...
Do not publish fixable findings, and do not add blanket `.trivyignore` entries without
individually justified non-applicability evidence.
